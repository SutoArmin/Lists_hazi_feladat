...
namespace Listak
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //2
            int osszeg = 0;
            foreach (var x  in szamsorA)
            {
                osszeg += x;
            }
            double atlag = (double)osszeg / szamsorA.Count;
            Console.WriteLine("szamsorA átlaga: " + atlag);
            ...
            while (i < szamsorA.Count && m < szamsorB.Count)
            {
                if (szamsorA[i] < szamsorB[m])
                {
                    ujSzamsor.Add(szamsorA[i]);
                    i++;
                }
                else
                {
                    ujSzamsor.Add(szamsorB[m]);
                    m++;
                }
            }

            while (i < szamsorA.Count)
            {
                ujSzamsor.Add(szamsorA[i]);
                i++;
            }
            while (m < szamsorB.Count)
            {
                ujSzamsor.Add(szamsorB[m]);
                m++;
            }

            Console.WriteLine("ujSzamsor elemei:");
            foreach (var szam in ujSzamsor)
            {
            ...
            //6
            Console.WriteLine("Epson Magenta patronok:");
            for (int k = 0; k < tintapatronok.Count; k++)
            {
                string patron = tintapatronok[k];
                if (patron.Contains("Epson") && patron.Contains("Magenta"))
                {
            ...
            //8
            int plusz = 0, minusz = 0, nulla = 0;

            for (int j = 0; j < jelek.Count; j++)
            {
                if (jelek[j] == '+') plusz++;
                else if (jelek[j] == '-') minusz++;
                else if (jelek[j] == '0') nulla++;
            }

            Console.WriteLine("Előfordulások: + = " + plusz + ", - =" + minusz + ", 0 = " + nulla);

            if (plusz >= minusz && plusz >= nulla)
                Console.WriteLine("Legtöbb jel: +");
            else if (minusz >= plusz && minusz >= nulla)
                Console.WriteLine("Legtöbb jel: -");
            else
                Console.WriteLine("Legtöbb jel: 0");
        }
    }
}
